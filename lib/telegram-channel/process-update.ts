...
export async function processInboundTelegramUpdate(
  ...
  // v2.1 Slice 6 (#137): group inbounds (photo + text) now flow through
  // the state-machine engine. `buildState` runs `parsePackagePhoto` for
  // photo (→ `group-photo-nudge` on shipping_label, `group-silent`
  // otherwise) and `classifyGroupMessage` + holder lookup + per-recipient
  // `registerPackage` for text (→ `group-silent`, `group-text-clarification`
  // for agent fallthrough, `group-text-holder-not-registered`, or
  // `group-text-registered`). The `group-text-clarification` variant is
  // the only group surface where the agent receives a free-text inbound,
  // and its synthetic constrains it to one short clarifying question.
  // The match arm emits `setTriggerAttribute("telegram.group")` +
  // `agent.start` trace + `sendToAsh` directly — so no fallthrough tail
  // is needed here.
  const state = await buildState({ kind: "group", message: inbound }, deps);
  const { actions } = match(state);
  await runActions(actions, deps);
  return new Response(null, { status: 204 });
}
