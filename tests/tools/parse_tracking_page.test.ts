 ...
 * the output schema's optional-field handling. Sibling of
 * `parse_label.test.ts`.
 *
 * Tests are intentionally model-agnostic: the AI Gateway's actual
 * routing is exercised behind `generateObject`, which we mock. We
 * assert on the call sequence (primary first, then fallback iff
 * primary throws) and on the model strings the tool passes to
 * `generateObject` so the fallback chain stays stable.
 *
 * Model-identifier regression guard: the slugs are imported from the
 * tool module rather than re-typed here. The vision-capable-slug
 * allowlist mirrors `parse_label.test.ts` — if Gemma 4 IT slugs or
 * any other text-only model sneaks in, the test fails.
 *
 ...
 * client converts them to a `data:` URI and the Gateway server
 * rejects with "Unsupported file URI type". Tests assert URL-shape.
 */
...
const sampleUrl =
  "https://api.telegram.org/file/bot111:AAA/photos/screenshot_42.jpg";
...
describe("parse_tracking_page", () => {
  ...
  it("returns the parsed tracking-page fields from the primary model on the happy path", async () => {
    ...
    expect(result.trackingNumber).toBe("DE92465739A");
    expect(result.expectedWindowStartAt).toBe("2026-05-19T12:00:00Z");
  ...
  it("passes the image URL through as a FilePart with mediaType='image' so the Gateway server fetches it", async () => {
    ...
    expect(textPart.text).toMatch(/caption.*screenshot/i);
  });

  it("PRIMARY_MODEL and FALLBACK_MODEL name vision-capable slugs on the AI Gateway", async () => {
    // Regression guard: text-only slugs (e.g. google/gemma-4-31b-it)
    // are valid Gateway model IDs but reject FilePart inputs at the
    // provider boundary. If the slugs ever drift to text-only, this
    // test fails before production does. Mirrors the allowlist in
    // tests/tools/parse_label.test.ts — keep both in sync when adding
    // a model.
    const { primary, fallback } = await loadModelSlugs();
  ...
  it("accepts FedEx as a recognized carrier in the output schema", async () => {
    // FedEx joined the carrier set in #50; this guard makes sure it
    // didn't drop off the parser's enum when the schema was authored.
    generateObjectMock.mockResolvedValueOnce({
      object: {
        carrier: "FedEx",
        trackingNumber: "1234567890",
        confidence: "high",
        reason: "FedEx logo legible",
      },
    });
